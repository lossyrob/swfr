...
// This one simply moves the files.
var localUpload = function(source, target) {
  return Promise
    .resolve(undefined)
    .then(function () {
...
var outputAsync = function outputAsync(source, target) {
  var uri = url.parse(target);

  if (uri.protocol === "s3:") {
    return s3Upload(source, target);
  } else {
    return localUpload(source, target);
  }
};

module.exports.outputAsync = outputAsync;
